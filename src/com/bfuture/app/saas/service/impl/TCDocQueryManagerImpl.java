    ...
    public ReturnObject getTCDocDetail(Object[] o){
        ...
        {   
        	...
        	if("3037".equals(tCDocQuery.getSgcode())){
        		 Sql = new StringBuffer("select A.btdgdid,A.btdbarcode,B.Gdname,B.Gdspec,B.Gdunit,A.btdsl,A.btdhsjj,A.btdhsjjje from yw_btcddet_fx A left join inf_goods B on a.btdsgcode = b.gdsgcode and a.btdgdid = b.gdid and a.btdbarcode = b.gdbarcode ");
        		 sumsql = new StringBuffer("select cast('\u5408\u8BA1' as varchar2(30)) btdgdid,sum(A.btdsl)btdsl,sum(A.btdhsjjje)btdhsjjje from yw_btcddet_fx A ");             	
        		 count =  new StringBuffer("select count(A.btdgdid) from yw_btcddet_fx A ");
        	}
