...
<head>
	...
	//查询未开户用户xjjjjjjjjjjjjjjj
		
		function search(){
			$('#searchUser_pl').window('open');
		}
		function cancelSearch(){
			$('#searchUser_pl').window('close');
		}
		function searchUser(){
			//查询参数直接添加在queryParams中
			var searchData = getFormData( 'searchUser_pl' );
			searchData['sgcode'] = User.sgcode; 
		    $('#userManager').datagrid('options').url = 'JsonServlet';        
			$('#userManager').datagrid('options').queryParams = {
				data :obj2str(
					{		
						ACTION_TYPE : 'getBatchOpenUserBySearch',
						ACTION_CLASS : 'com.bfuture.app.saas.model.SysScmuser',
						ACTION_MANAGER : 'userManager',	
						list:[
							searchData
...
<body>	
	<center>
	<div style="width:800px;height:560 ;position:relative ;">	
	...
	<div id="searchUser_pl" class="easyui-window" iconCls="icon-search" minimizable="false" maximizable="false" collapsible="false" shadow="true" closed="true" modal="true" title="查询用户" style="width:270px;height:160px;">
		<div class="easyui-layout" fit="true"> 
			<div region="center" border="false" style="padding:10px;background:#fff;"> 
				<table id="userManager_form" class="tableClass">
					<tr>
						<td>供应商编码：</td>
						<td><input name="supcode" type="text" /></td>						
					</tr>					
					<tr>
						<td>供应商名称：</td>
